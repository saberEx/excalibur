...
* @Last Modified time: 2016-08-12 15:55:53
* app头部bar组件
* 用法：
* TopBar.create('标题');
...
module.exports = {
    ...
    create:function (s_title,o_params) {
        ...
        if(!e_appBarCon){
            ...
            var containerHChange = Base.isNeedImmerse()?64:44;
            if(document.querySelector('.bottomTab')){
                containerHChange += 50;
            }
    ...
    hideTopBar:function() {
        ...
        ReactDOM.unmountComponentAtNode(e_appBarCon);
        e_appBarCon.parentNode.removeChild(e_appBarCon);
        e_appBarCon = null;
    }
};
function renderDom(s_title,o_params){
    ...
    if(s_leftIcon){
        var f_onClick = function(){
            var f_leftClick = o_params.f_leftClick;
            f_leftClick ? f_leftClick() : Base.closeWin();
        };
        e_leftIconEl = <div className="leftIcon" onClick={f_onClick}><img className="base-fl leftIcon" src={s_leftIcon} ></img></div>;
    }
