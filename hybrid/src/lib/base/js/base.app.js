...
var o_base = {
    ...
    download:function(s_path,f_callback){
        var savePath = '';
        s_path = decodeURIComponent(s_path);
        if(/.*ticket=/.test(s_path)){
            savePath = "fs://" + s_path.replace(/.*ticket=/,"").replace(/==/,"") + '.jpg';
        }
        api.download({
            url: s_path,
            savePath:savePath,
    ...
    //获取实际像素(设计稿像素转为实际像素)
    getPxReality:function(n_pxValue){
        return n_pxValue/32 * parseFloat(document.querySelector("html").style.fontSize);
    },
    replaceHtml(s_str){
        return s_str.replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&quot;/g,"'");
    }
};
//对外接口
